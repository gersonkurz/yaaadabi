...
func TestEnsureSettingsMergesWithoutDuplicates(t *testing.T) {
	...
	if _, err := ensureSettings(dir, []string{"Bash(just:*)"}); err != nil {
		t.Fatal(err)
	}
	// Second run must be a no-op.
	msg, err := ensureSettings(dir, []string{"Bash(just:*)"})
	...
	for _, want := range append(baseAllow, "Bash(dotnet build:*)", "Bash(just:*)") {
		if count[want] != 1 {
...
func TestEnsureSettingsRefusesInvalidJSON(t *testing.T) {
	...
	if _, err := ensureSettings(dir, nil); err == nil {
...
func TestEnsureClaudeMDRefusesPartialWiring(t *testing.T) {
	for name, content := range map[string]string{
		...
		if _, err := ensureClaudeMD(dir, "x"); err == nil {
			t.Errorf("%s: expected error, got none", name)
		}
	}
}
...
func TestEnsureClaudeMDAppendsOnceAndCreates(t *testing.T) {
	...
	if _, err := ensureClaudeMD(dir, "`make test`"); err != nil {
		t.Fatal(err)
	}
	msg, err := ensureClaudeMD(dir, "`make test`")
	if err != nil || !strings.Contains(msg, "already wired") {
		t.Fatalf("expected idempotent second run, got %q, %v", msg, err)
	}
	raw, _ := os.ReadFile(filepath.Join(dir, "CLAUDE.md"))
	s := string(raw)
	if !strings.HasPrefix(s, "# Existing") || strings.Count(s, importLine) != 1 || !strings.Contains(s, "`make test`") {
		t.Fatalf("unexpected CLAUDE.md content:\n%s", s)
	}

	// Missing CLAUDE.md gets created.
	dir2 := t.TempDir()
	if _, err := ensureClaudeMD(dir2, "x"); err != nil {
		t.Fatal(err)
	}
